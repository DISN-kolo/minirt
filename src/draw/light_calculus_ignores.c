...
/*   Updated: 2024/07/29 16:32:22 by akozin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ignore_light_cy(t_data *data, int oi, int j)
{
	...
	sign_d = (dot_prod(vec_sub(vec_add(data->objs[oi].origin,
				...
				data->objs[oi].normal) > 0);
	if ((sign_a < 0 && sign_b < 0) || (sign_c > 0 && sign_d > 0))
		return (0);
	cam_dist_v = vec_sub(data->cam.origin, data->objs[oi].origin);
	cam_dist = vec_len(vec_sub(vec_scale(data->objs[oi].normal, dot_prod(cam_dist_v, data->objs[oi].normal)),
				cam_dist_v));
	light_dist_v = vec_sub(data->lights[j].origin, data->objs[oi].origin);
	light_dist = vec_len(vec_sub(vec_scale(data->objs[oi].normal, dot_prod(light_dist_v, data->objs[oi].normal)),
				light_dist_v));
